package com.WazaBe.GooglePlusStream;
 ...
 * 
 * <p>
 * WARNING! This fragment uses the Google+ API, and is subject to quotas. If you
 * expect to write a wildly popular app based on this code, please check the at
 * <a href="https://developers.google.com/+/">Google+ Platform documentation</a>
 * on latest best practices and quota details. You can check your current quota
 * at the <a href="https://code.google.com/apis/console">APIs console</a>.
 */
public class SocialStreamFragment extends SherlockListFragment implements
	...
	// Configure keyword here:
	private static final String KEYWORD = "Waza_be";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final Intent intent = fragmentArgumentsToIntent(getArguments());
		...
		mImageFetcher = getImageFetcher(getActivity());
	 ...
	 * (item.getItemId()) { case R.id.menu_compose: Intent intent =
	 ...
	 * "Post to Google+", 0L); LOGD("Tracker",
	 * "Home Screen Dashboard: Click, post to Google+");
	 * 
	...
	public void onSaveInstanceState(Bundle outState) {
		if (isAdded()) {
			...
			outState.putInt(STATE_POSITION, getListView()
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		/*
		 * Intent postDetailIntent = new Intent(Intent.ACTION_VIEW,
		 * Uri.parse(activity.getUrl()));
		 * postDetailIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET
		 * ); preferPackageForIntent(getActivity(), postDetailIntent,
		 * "com.google.android.apps.plus"); safeOpenLink(getActivity(),
		 * postDetailIntent);
		 */
	}
	...
	public void onScroll(AbsListView absListView, int firstVisibleItem,
		...
		// Simple implementation of the infinite scrolling UI pattern; loads
		// more Google+
		// search results as the user scrolls to the end of the list.
		if (!isStreamLoading() && streamHasMoreResults()
				&& visibleItemCount != 0
	...
	@Override
	public void onLoadFinished(Loader<List<Activity>> listLoader,
		...
		if (mListViewStatePosition != -1 && isAdded()) {
			getListView().setSelectionFromTop(mListViewStatePosition,
					mListViewStateTop);
			mListViewStatePosition = -1;
		}
	}
	...
	private static class StreamLoader extends AsyncTaskLoader<List<Activity>> {
		...
		public List<Activity> loadInBackground() {
			...
			Plus plus = Plus.builder(httpTransport, jsonFactory)
					.setApplicationName("WazaBe - Developement tests")
			...
			try {
				activities = plus.activities().search(mSearchString)
						...
						.setMaxResults(MAX_RESULTS_PER_REQUEST).execute();
		...
		public void deliverResult(List<Activity> activities) {
			...
			if (isStarted()) {
				...
				super.deliverResult(mActivities == null ? null
						: new ArrayList<Activity>(mActivities));
			}
		}
	...
	/**
	 * A list adapter that shows individual Google+ activities as list items. If
	 * another page is available, the last item is a "loading" view to support
	 * the infinite scrolling UI pattern.
	 */
	private class StreamAdapter extends BaseAdapter {
		...
		public int getItemViewType(int position) {
			return (position >= mStream.size()) ? VIEW_TYPE_LOADING
					: VIEW_TYPE_ACTIVITY;
		}

		@Override
		public Object getItem(int position) {
			return (getItemViewType(position) == VIEW_TYPE_ACTIVITY) ? mStream
					.get(position) : null;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (getItemViewType(position) == VIEW_TYPE_LOADING) {
				...
				StreamRowViewBinder.bindActivityView(convertView, activity,
						mImageFetcher);
				return convertView;
			}
		}
	...
	/**
	 * A helper class to bind data from a Google+ {@link Activity} to the list
	 * item view.
	 */
	private static class StreamRowViewBinder {
		...
		private static void bindActivityView(final View rootView,
				Activity activity, ImageFetcher imageFetcher) {
			// Prepare view holder.
			...
			if (temp != null) {
				...
				views.detail = new TextView[] { (TextView) rootView
						.findViewById(R.id.stream_detail_text) };
				views.detailIcon = new ImageView[] { (ImageView) rootView
						.findViewById(R.id.stream_detail_media_small) };
				views.media = new ImageView[] {
						(ImageView) rootView
								.findViewById(R.id.stream_media_1_1),
						(ImageView) rootView
								.findViewById(R.id.stream_media_1_2), };
				views.mediaOverlay = new ImageView[] {
						(ImageView) rootView
								.findViewById(R.id.stream_media_overlay_1_1),
						(ImageView) rootView
								.findViewById(R.id.stream_media_overlay_1_2), };
				views.originalAuthor = (TextView) rootView
						.findViewById(R.id.stream_original_author);
				views.reshareLine = rootView
						.findViewById(R.id.stream_reshare_line);
				views.reshareSpacer = rootView
						.findViewById(R.id.stream_reshare_spacer);
				views.userImage = (ImageView) rootView
						.findViewById(R.id.stream_user_image);
				views.userName = (TextView) rootView
						.findViewById(R.id.stream_user_name);
				views.content = (TextView) rootView
						.findViewById(R.id.stream_content);
				views.plusOneIcon = rootView
						.findViewById(R.id.stream_plus_one_icon);
				views.plusOneCount = (TextView) rootView
						.findViewById(R.id.stream_plus_one_count);
				views.commentIcon = rootView
						.findViewById(R.id.stream_comment_icon);
				views.commentCount = (TextView) rootView
						.findViewById(R.id.stream_comment_count);
			}
			...
			// Determine if this is a reshare (affects how activity fields are
			// to be
			// interpreted).
			...
			if (activity.getActor().getImage() != null) {
				imageFetcher.loadThumbnailImage(activity.getActor().getImage()
						.getUrl(), views.userImage,
			...
			final int plusOneCount = (activity.getObject().getPlusoners() != null) ? activity
					.getObject().getPlusoners().getTotalItems().intValue()
					: 0;
			if (plusOneCount == 0) {
			...
			final int commentCount = (activity.getObject().getReplies() != null) ? activity
					.getObject().getReplies().getTotalItems().intValue()
					: 0;
			if (commentCount == 0) {
			...
			// Set content.
			String selfContent = isReshare ? activity.getAnnotation()
			...
			if (activity.getObject().getActor() != null) {
				...
				views.originalAuthor.setText(res.getString(
						R.string.stream_originally_shared, activity.getObject()
			...
			if (isReshare
					&& !TextUtils.isEmpty(activity.getObject().getContent())
					&& detailIndex < views.detail.length) {
				views.detail[detailIndex].setVisibility(View.VISIBLE);
				views.detail[detailIndex].setTextColor(res
						.getColor(R.color.stream_content_color));
				views.detail[detailIndex].setText(Html.fromHtml(activity
			...
			if (!TextUtils.isEmpty(location)
				...
				views.detail[detailIndex].setVisibility(View.VISIBLE);
				views.detail[detailIndex].setTextColor(res
						.getColor(R.color.stream_link_color));
				...
				if ("checkin".equals(activity.getVerb())) {
					views.detailIcon[detailIndex]
							.setImageResource(R.drawable.stream_ic_checkin);
				} else {
					views.detailIcon[detailIndex]
							.setImageResource(R.drawable.stream_ic_location);
				}
			...
			if (activity.getObject().getAttachments() != null) {
				for (Activity.PlusObject.Attachments attachment : activity
						.getObject().getAttachments()) {
					String objectType = attachment.getObjectType();
					if (("photo".equals(objectType) || "video"
							.equals(objectType))
						...
						imageFetcher.loadThumbnailImage(attachment.getImage()
								.getUrl(), mediaView);

						if ("video".equals(objectType)) {
							views.mediaOverlay[mediaIndex]
									.setVisibility(View.VISIBLE);
						}
						...
						try {
							...
							imageFetcher.loadThumbnailImage(faviconUrl,
									iconView);
						...
						views.detail[detailIndex].setTextColor(res
								.getColor(R.color.stream_link_color));
						views.detail[detailIndex].setText(attachment
								.getDisplayName());
